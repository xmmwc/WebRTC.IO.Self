...
function attachEvents(manager){
    ...
    manager.sockets.on('connection',function(socket){
        ...
        socket.on('join_room',function(room){
            ...
            var time = new Date();
            console.log('new_peer_connected to other in ' + room + 'id is ' + socket.id);
            console.log('+ ip is ' + socket.handshake.address + ' time: ' +
        ...
        socket.on('send_non_stream',function(data){
            var sendTo = getSocket(data.socketId);
            if(sendTo) {
                console.log('send_non_stream to ' + data.socketId);
                sendTo.emit('receive_non_stream', {
                    socketId: socket.id
                })
            }
        });
